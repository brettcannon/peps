...
:file:`.python-envs` file. IF a path is for a virtual environment, THEN path
MUST be to the directory of the virtual environment (i.e. the directory
...
IF both a virtual environment in a :file:`.venv` directory and a
:file:`.python-envs` file exist side-by-side, THEN the :file:`.venv` directory
...
even use the :file:`.venv` name. But in those situations, you need _some_ way
...
XXX allowing any type of environment
XXX allowing any representation
XXX allowing duplicates
XXX why the last entry is the default
XXX why .venv is implicitly at the end of .python-envs

XXX trailing newline
XXX getting virtual environment name from the prompt
